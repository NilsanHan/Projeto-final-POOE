...
public class Conexao implements SerialPortEventListener {
	...
	// private static List<String> resultList = Collections.synchronizedList(new
	// ArrayList<>());
	...
	public void serialEvent(SerialPortEvent event) {
		...
		if (event.getEventType() == SerialPortEvent.DATA_AVAILABLE) {
			try {
				int resultRecebido = leitura.read();
				System.out.println("recebendo dados na serial " + resultRecebido);
				setResult(resultRecebido);

				ControleService controleService = new ControleService();
				Controle controle = new Controle();
				if (resultRecebido == 49)
					controle.setEstado("aberto");
				else
					controle.setEstado("fechado");
				controle.setHorario(new Date());
